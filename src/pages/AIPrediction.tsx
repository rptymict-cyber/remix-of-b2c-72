    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="px-4 pt-5 safe-bottom space-y-5">
        ...
        <div className="bg-card rounded-lg border border-border p-3 space-y-2.5">
          <p className="text-xs font-semibold text-foreground">분석 조건</p>
          <div className="flex flex-wrap gap-2">
            <button onClick={() => setCropOpen(true)} className="condition-chip">
              <span className="text-sm">{crop.emoji}</span>
              {crop.name} <ChevronDown className="w-3 h-3 text-muted-foreground" />
            </button>
            <button onClick={() => setVarietyOpen(true)} className="condition-chip">
              <Layers className="w-3.5 h-3.5 text-muted-foreground" />
              {variety} <ChevronDown className="w-3 h-3 text-muted-foreground" />
            </button>
            <button onClick={() => setMarketOpen(true)} className="condition-chip">
              <Building2 className="w-3.5 h-3.5 text-muted-foreground" />
              {market.name} <ChevronDown className="w-3 h-3 text-muted-foreground" />
            </button>
          </div>
          <div className="flex gap-2">
            <button onClick={() => setQtyOpen(true)} className="condition-chip flex-1">
              <Package className="w-3.5 h-3.5 text-muted-foreground" />
              {quantity}상자 · {unitWeight}kg <ChevronDown className="w-3 h-3 text-muted-foreground" />
            </button>
